...
<c:if test="${code == searchForm.sortColumn}">
    <c:if test="${searchForm.sortDirection == 'asc'}">
        <c:set var="sortIcon" value="arrow-up"/>
        <c:set var="newDirection" value="desc"/>
    </c:if>
    <c:if test="${searchForm.sortDirection == 'desc'}">
        <c:set var="sortIcon" value="arrow-down"/>
        <c:set var="newDirection" value="asc"/>
    </c:if>
</c:if>

<th id="${code}${searchForm.id}" class="nowrap ${cssClass}" style="cursor: pointer; ${style}"
    onclick="ajaxItems(this.id, '${pageContext.servletConfig.servletContext.contextPath}', '${searchForm.endpoint}',
            '${searchForm.id}', '${searchForm.page}', '${code}', '${newDirection}');">
    ${label}
