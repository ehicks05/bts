...
<nav class="pagination is-centered" role="navigation" aria-label="pagination">
    ...
    <a class="pagination-previous" id="previous${searchForm.id}" ${disabled}
       onclick="ajaxItems(this.id, '${pageContext.request.contextPath}', '${searchForm.endpoint}', '${searchForm.id}', '${searchForm.page - 1}', '${searchForm.sortColumn}', '${searchForm.sortDirection}')"><i class="fas fa-chevron-left"></i></a>

    <c:set var="disabled" value="${!searchResult.hasNext ? 'disabled' : ''}" />
    <a class="pagination-next" id="next${searchForm.id}" ${disabled}
       onclick="ajaxItems(this.id, '${pageContext.request.contextPath}', '${searchForm.endpoint}', '${searchForm.id}', '${searchForm.page + 1}', '${searchForm.sortColumn}', '${searchForm.sortDirection}')"><i class="fas fa-chevron-right"></i></a>
        ...
        <c:forEach var="navPage" items="${searchResult.navPages}">
            ...
            <li>
                <a id="page${navPage}${searchForm.id}" class="pagination-link ${isCurrent}" aria-label="Goto page ${navPage}"
                   onclick="ajaxItems(this.id, '${pageContext.request.contextPath}', '${searchForm.endpoint}', '${searchForm.id}', '${navPage}', '${searchForm.sortColumn}', '${searchForm.sortDirection}')"
                >${navPage}</a>
            </li>
        </c:forEach>
    </ul>
